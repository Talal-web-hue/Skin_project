...
Route::get('orders/{id}/show', [OrderController::class, 'show'])->middleware('auth:sanctum');
...
Route::delete('deleteOrder/{id}' , [OrderController::class , 'delete'])->middleware('auth:sanctum');    

// راوت حساب الفاتورة
/* رح يكون عام أي يمكن الزبون حساب الفاتورة لمعرفة التكلفة الإجمالية
قبل إنشاء حساب او إتمام الشراء
أيضا يمكن للآدمن لحساب فواتير الطلبات الهاتفية أو الخاصة نيابة عن العملاء
*/
Route::post('order/calculate_invoice' , [OrderController::class , 'calculateInvoice'  , 'calculateInvoice']); // لا يحتاج إلى صلاحيات
...
// Review API
Route::post('reviewStore' , [ReviewController::class , 'store'])->middleware('auth:sanctum');  // للعميل فقط
Route::delete('reviewDelete/{id}' , [ReviewController::class , 'delete'])->middleware('auth:sanctum');  // للعميل فقط
Route::get('review/{type}/{id}' , [ReviewController::class , 'index']);  // سيكون عام , أي للزوار و العملاء